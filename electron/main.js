...
function normalizeCryptoProfile(config = null) {
  const iterations = Number(config && config.kdfIterations)
...
function saveCryptoProfile(config) {
...
function createCryptoProfileBackup(profile) {
  return {
...
function parseCryptoProfileBackup(value) {
...
function sendRendererNavigation(route) {
  if (!route || !route.startsWith('/') || !mainWindow || mainWindow.isDestroyed()) return
...
ipcMain.handle('backend-config:set', (_event, config) => {
...
ipcMain.handle('crypto-profile:set', (_event, config) => {
...
ipcMain.handle('crypto-profile:export-backup', async (_event, profile) => {
