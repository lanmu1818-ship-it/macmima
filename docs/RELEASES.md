...
- 新增 Markdown 文档分类，可保存 API 文档、请求示例和代码块，并支持共享到共享密区。
- 新增开发讨论，支持图片、`@` 在线成员、消息提醒、按日期折叠和分页加载历史记录。
- 新增 Crypto v2：个人密区支持本机增强密钥，共享密区支持独立共享密区密钥。
- 优化复制内容，Markdown 文档复制时减少无关元数据。
- 更新官网和开源文档，重点解释后端只保存密文、元数据边界和自托管部署注意事项。
