...
- Express 后端负责账号、权限、同步和密文储存。
- MySQL 储存用户、工作台、邀请码、凭证密文、凭证元数据和历史版本。
...
| 字段类别 | 例子 | 明文可见性 |
| --- | --- | --- |
| 凭证正文 | 网站密码、数据库密码、API Key、SSH 私钥、连接字符串、数据库表说明 | 不明文保存，进入后端前已加密 |
...
共享密区要透明说明一个边界：工作台 Key 既用于后端工作台隔离，也用于当前版本的共享密钥派生。
如果后端运行环境被完全入侵，攻击者可能通过请求链路或日志错误配置捕获工作台 Key，
从而增加共享密区风险。个人密区仍依赖用户主密码派生密钥，不依赖工作台 Key。
...
| 后端完全被入侵 | 可能影响认证、同步和共享密区安全；应轮换工作台 Key、JWT、pepper 和数据库密码 |
