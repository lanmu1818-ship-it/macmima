...
MacMima Crypto v2 在兼容旧数据的基础上，为新保存或编辑的个人凭证增加“本地增强密钥”：

- 本地增强密钥只保存在桌面端本机，不上传后端。
- Electron 环境会优先使用系统 `safeStorage` 加密保存本地配置。
- 新 v2 个人密文需要“主密码派生密钥 + 本地增强密钥”共同参与解密。
- 如果只泄露后端数据库，攻击者缺少本地增强密钥，离线解密难度更高。
- 如果用户更换电脑，需要先备份或导入本地增强密钥；丢失后无法解密使用它加密的数据。

v1 老数据仍然可读。用户不需要一次性迁移数据库。
...
| 模式 | 说明 |
| --- | --- |
| v1 兼容模式 | 使用工作台 Key 派生共享密钥，用于读取旧共享数据或未配置共享加密密钥的团队 |
| Crypto v2 | 使用独立的“共享密区加密密钥”，团队成员在本机配置同一个值，新共享数据使用该密钥加密 |

Crypto v2 的共享密区加密密钥不会作为 `X-MacMima-Workspace-Key` 请求头发送给后端。
工作台 Key 继续用于后端工作台隔离；共享密区加密密钥只用于客户端加解密。

边界是：

- 只泄露数据库时，攻击者仍然拿不到共享密区明文，因为数据库里没有工作台 Key 明文。
- 对 v2 新共享数据来说，仅捕获工作台 Key 仍不足以解密共享密区。
- 如果团队把共享密区加密密钥发到后端日志、聊天记录或公开仓库中，v2 保护会失效。
- 如果恶意客户端在加密前读取明文，这是任何客户端加密工具都需要面对的供应链风险。

后续路线仍然是改为“每个用户公钥包装共享 vault key”，进一步减少团队成员手动分发共享密钥的成本。
...
- v1 共享密区可能受工作台 Key 暴露影响；v2 共享密区还依赖独立共享加密密钥。
...
- 如果怀疑共享密区加密密钥泄露，重新生成共享密钥，并编辑/重存共享凭证使其重新加密。
...
- 将共享密区从手动共享加密密钥升级为每用户公钥包装共享 vault key。
- 增加加密本地备份和恢复密钥流程。
