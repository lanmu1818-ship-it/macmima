...
function normalizeImportedCryptoProfile(profile: Partial<StoredCryptoProfile>): StoredCryptoProfile {
  const iterations = Number(profile.kdfIterations)
  ...
  return {
    ...
    kdfIterations:
      Number.isFinite(iterations) && iterations >= 100000 && iterations <= 1000000
        ...
        : 210000,
    localSecret: typeof profile.localSecret === 'string' ? profile.localSecret.trim() : '',
    sharedVaultSecret:
      typeof profile.sharedVaultSecret === 'string' ? profile.sharedVaultSecret.trim() : '',
    updatedAt: typeof profile.updatedAt === 'string' ? profile.updatedAt : undefined,
  }
}

function createCryptoProfileBackup(profile: StoredCryptoProfile): CryptoProfileBackup {
  return {
    schema: 'macmima.crypto-profile.backup',
    version: 1,
    exportedAt: new Date().toISOString(),
    profile: normalizeImportedCryptoProfile(profile),
  }
}

function parseCryptoProfileBackup(value: unknown): StoredCryptoProfile {
  ...
  const profile = normalizeImportedCryptoProfile(backup.profile)
...
export default function SettingsPage() {
  ...
  const [localApiDraft, setLocalApiDraft] = useState<LocalApiConfig>(DEFAULT_LOCAL_API_CONFIG)
  ...
  const [cryptoProfile, setCryptoProfileState] = useState<StoredCryptoProfile>(defaultCryptoProfile)
  ...
  const cryptoBackupInputRef = useRef<HTMLInputElement | null>(null)
...
${localApiDocumentCurlCopy}`
  ...
  const handleExportCryptoBackup = async () => {
    const profile = normalizeImportedCryptoProfile(cryptoProfileDraft)

    if (!profile.localSecret && !profile.sharedVaultSecret) {
      setCryptoProfileMessage('当前没有可导出的加密密钥')
      return
    }
    ...
    try {
      ...
      if (window.electronAPI?.exportCryptoProfileBackup) {
        const result = await window.electronAPI.exportCryptoProfileBackup(profile)
        if (result.canceled) return
        setCryptoProfileMessage('已导出加密配置备份。请妥善保存，泄露后可能影响数据安全。')
        return
      }

      const backup = createCryptoProfileBackup(profile)
      downloadJsonFile(
        `macmima-crypto-backup-${backup.exportedAt.replace(/[:.]/g, '-')}.json`,
        backup
      )
      setCryptoProfileMessage('已导出加密配置备份。请妥善保存，泄露后可能影响数据安全。')
    } catch (error: any) {
      setCryptoProfileMessage(error.message || '导出备份失败')
    }
  }

  const applyImportedCryptoProfile = async (profile: StoredCryptoProfile) => {
    const savedProfile = await saveCryptoProfile(profile)
    setCryptoProfileState(savedProfile)
    setCryptoProfileDraft(savedProfile)
    setShowLocalSecret(Boolean(savedProfile.localSecret))
    setShowSharedVaultSecret(Boolean(savedProfile.sharedVaultSecret))
    setCryptoProfileMessage('已导入并保存加密配置。个人密区会使用导入的本地增强密钥解密。')
  }

  const handleImportCryptoBackup = async () => {
    if (
      !confirm(
        '导入备份会覆盖当前加密配置。请确认备份来自可信来源，并且是您要恢复的 MacMima Crypto v2 备份。'
      )
    ) {
      return
    }
    ...
    try {
      ...
      if (window.electronAPI?.importCryptoProfileBackup) {
        const result = await window.electronAPI.importCryptoProfileBackup()
        if (result.canceled) return
        if (result.profile) {
          setCryptoProfileState(result.profile)
          setCryptoProfileDraft(result.profile)
          setShowLocalSecret(Boolean(result.profile.localSecret))
          setShowSharedVaultSecret(Boolean(result.profile.sharedVaultSecret))
        }
        setCryptoProfileMessage('已导入并保存加密配置。个人密区会使用导入的本地增强密钥解密。')
        return
      }

      cryptoBackupInputRef.current?.click()
    } catch (error: any) {
      setCryptoProfileMessage(error.message || '导入备份失败')
    }
  }

  const handleCryptoBackupFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]
    event.target.value = ''
    if (!file) return
    ...
    try {
      ...
      await applyImportedCryptoProfile(parseCryptoProfileBackup(JSON.parse(content)))
    } catch (error: any) {
      setCryptoProfileMessage(error.message || '导入备份失败')
    }
  }
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-gray-500">
                    AES-256-GCM +
                    PBKDF2-SHA256。算法公开，密钥私有；本地增强密钥和共享密区加密密钥不会上传后端。
                  </p>
                </div>
              </div>
              <label className="inline-flex shrink-0 items-center gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  ...
                  onChange={(event) => updateCryptoProfileDraft({ enabled: event.target.checked })}
            ...
            <div className="max-w-xl">
              <div>
                <label className="mb-2 block text-sm font-medium text-gray-700">KDF 迭代次数</label>
                <input
                  type="number"
                  min={100000}
                  max={1000000}
                ...
                <div className="mb-2 flex items-center justify-between gap-3">
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        handleCopy('localCryptoSecret', cryptoProfileDraft.localSecret)
                      }
                      disabled={!cryptoProfileDraft.localSecret}
                      className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-2.5 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-40"
                    >
                      {copiedField === 'localCryptoSecret' ? (
            ...
            <input
              ref={cryptoBackupInputRef}
              type="file"
              accept="application/json,.json"
              className="hidden"
              onChange={handleCryptoBackupFile}
            />
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="inline-flex h-10 items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-100"
              >
              ...
              <button
                type="button"
                onClick={handleImportCryptoBackup}
                className="inline-flex h-10 items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-100"
              >
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs">
                ...
                <div className="rounded-md border border-gray-200 bg-white px-3 py-2 md:col-span-2">
                  ...
                  <p className="font-mono text-gray-900 mt-1 break-all">{localApiHeaderValue}</p>
                </div>
              </div>
